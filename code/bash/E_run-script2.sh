...
proc=0
#download
for data in `(cat $file | grep -v download)`
do
...
#sleep 100
#reboot
for data in `(cat $file | grep download | grep -v reboot)`
do
 ip=`(echo $data | cut -f 1 -d ";")`
 model=`(echo $data | cut -f 2 -d ";")`
 firmware=`(echo $data | cut -f 3 -d ";")`
 ...
 fun_run_script 2>&1 | tee ./temp/"$ip"_reboot.log || continue
 (($proc%15)) || { sleep 3;continue;}
 (($proc%75)) || wait
done
